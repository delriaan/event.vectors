...
do.load_unloaded("book.of.features, foreach, doFuture, igraph, matrixStats, plotly, purrr");

registerDoFuture();
plan(sequential);
plan(tweak(multisession, workers = 5));

dir("experiments", pattern = "test", full.names = TRUE) |> data.table::last() |> load(verbose = TRUE);
